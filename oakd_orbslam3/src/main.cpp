...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        auto rectif_left_frame = rectif_left_queue->tryGet<dai::ImgFrame>();
        auto rectif_right_frame = rectif_right_queue->tryGet<dai::ImgFrame>();

        if (!rectif_left_frame || !rectif_right_frame) {
            std::cerr << "Frames not received!" << std::endl;
            continue;
        }
        ...
        cv::Mat rectif_left_gray, rectif_right_gray;

        if (rectif_left.channels() == 3)
            cv::cvtColor(rectif_left, rectif_left_gray, cv::COLOR_BGR2GRAY);
        else
            rectif_left_gray = rectif_left;

        if (rectif_right.channels() == 3)
            cv::cvtColor(rectif_right, rectif_right_gray, cv::COLOR_BGR2GRAY);
        else
            rectif_right_gray = rectif_right;
        ...
        if (rectif_left.empty() || rectif_right.empty()) {
            std::cerr << "One of the stereo images is empty!" << std::endl;
            continue;
        }
        else{
            std::cerr << "No issues with the images being empty" << std::endl;
            continue;
        }

        std::cout << "Left image size: " << rectif_left_gray.cols << "x" << rectif_left_gray.rows
          << ", type: " << rectif_left_gray.type() << std::endl;
        std::cout << "Right image size: " << rectif_right_gray.cols << "x" << rectif_right_gray.rows
          << ", type: " << rectif_right_gray.type() << std::endl;


        cv::Mat raw_pose = SLAM.TrackStereo(rectif_left_gray, rectif_right_gray, frame_timestamp_s);
